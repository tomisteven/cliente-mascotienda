    ...
    <div className="space-y-6 pb-10">
        ...
        <button
          ...
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
            !categoryFilter
              ...
              : 'bg-surface text-textMuted hover:text-textLight border border-slate-700'
          }`}
        >
          Todas
        </button>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <div className="bg-surface p-4 rounded-xl border border-slate-800">
          ...
          <p className="text-lg font-bold text-slate-400">{formatCurrency(summary.costoTotal)}</p>
        </div>
        <div className="bg-surface p-4 rounded-xl border border-slate-800">
        ...
        <div className="bg-surface p-4 rounded-xl border border-slate-800">
          ...
          <p className="text-lg font-bold text-textLight flex items-center">
            <ShoppingCart size={16} className="text-blue-400 mr-1" />
            {summary.totalVentas}
          </p>
        </div>
        <div className="bg-surface p-4 rounded-xl border border-slate-800">
          ...
          <p className="text-lg font-bold text-textLight flex items-center">
            <Package size={16} className="text-purple-400 mr-1" />
        ...
        <div className="p-4 border-b border-slate-800 flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="bg-background border border-slate-700 rounded-lg pl-9 pr-3 py-2 text-sm text-textLight focus:outline-none focus:border-primary w-48 sm:w-64"
            />
          </div>
        </div>
              ...
              {sortedProducts.map((p, i) => {
                ...
                return (
                  <tr key={p._id} className="hover:bg-slate-800/40 transition-colors">
                    <td className="px-4 py-3 font-bold text-slate-500 text-center">#{actualRank}</td>
                    ...
                    <td className="px-4 py-3 text-right text-blue-400">{p.ingresosUnidad > 0 ? formatCurrency(p.ingresosUnidad) : <span className="text-slate-600">—</span>}</td>
                    <td className="px-4 py-3 text-right text-amber-400">{p.ingresosSuelta > 0 ? formatCurrency(p.ingresosSuelta) : <span className="text-slate-600">—</span>}</td>
                    <td className="px-4 py-3 text-right text-slate-400">{formatCurrency(p.costoTotal)}</td>
