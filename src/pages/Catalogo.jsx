          ...
          <div className="mt-10 max-w-xl mx-auto">
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-white/5 border border-slate-700/50 rounded-2xl pl-12 pr-4 py-3.5 text-white placeholder-slate-500 focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 focus:outline-none transition-all backdrop-blur-sm"
              />
            </div>
          </div>
            ...
            <button
              ...
              className={`px-5 py-2 rounded-full text-sm font-bold whitespace-nowrap transition-all border ${
                selectedCategory === 'all'
                  ...
                  : 'bg-white/5 border-slate-700/50 text-slate-400 hover:bg-white/10 hover:text-slate-200'
              }`}
            >
              Todos
            </button>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20">
        {loading ? (
          <div className="flex flex-wrap gap-6 justify-center">
            {[...Array(8)].map((_, i) => (
              <div key={i} className="w-full sm:w-[280px] bg-white/5 rounded-2xl border border-slate-700/50 overflow-hidden animate-pulse">
          ...
          <>
            <div className="flex items-center justify-between mb-6">
              <p className="text-slate-500 text-sm">
                Mostrando <span className="text-slate-300 font-semibold">{filtered.length}</span> producto{filtered.length !== 1 ? 's' : ''}
              </p>
            </div>
            <div className="flex flex-wrap gap-6 justify-center sm:justify-start">
              {filtered.map(p => (
                <div
                  key={p._id}
                  className="group w-full sm:w-[280px] bg-white/5 rounded-2xl border border-slate-700/50 overflow-hidden hover:border-slate-600/50 hover:bg-white/[0.07] transition-all duration-300 hover:shadow-xl hover:shadow-indigo-500/5 hover:-translate-y-1"
                >
                  <div className="relative h-14 bg-gradient-to-r from-slate-800/80 to-slate-900/80 flex items-center px-5">
                    {p.categoria && (
                      <span
                  ...
                  <div className="p-5 pt-4">
                    ...
                    {p.unidadMedida && p.unidadMedida !== 'unidad' && (
                      <p className="text-[10px] text-slate-600 mt-2 uppercase tracking-wider">Venta por {p.unidadMedida}</p>
                    )}
                  </div>
                </div>
              ))}
      ...
      <footer className="border-t border-slate-800/50 py-8">
        <div className="max-w-7xl mx-auto px-4 text-center">
          <p className="text-slate-600 text-sm">
            &copy; {new Date().getFullYear()} Mascotienda — Todos los precios pueden variar sin previo aviso
          </p>
        </div>
      </footer>
    </div>
