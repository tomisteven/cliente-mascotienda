  ...
  const renderModal = () => {
    ...
    return (
      <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setModal(null)}>
        <div className="bg-surface rounded-2xl border border-slate-800 w-full max-w-lg max-h-[85vh] overflow-y-auto shadow-2xl" onClick={e => e.stopPropagation()}>
              ...
              <>
                <div>
                  ...
                  <select value={form.proveedor} onChange={e => setForm({ ...form, proveedor: e.target.value })}
                    className="w-full bg-background border border-slate-700 rounded-lg px-3 py-2 text-sm text-textLight focus:outline-none focus:border-primary">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {form.items?.map((item, idx) => (
                      <div key={idx} className="flex gap-2 items-start bg-background/50 p-2 rounded-lg border border-slate-700/50">
                        <div className="flex-1 min-w-0">
                          <select value={item.producto} onChange={e => updateOrderItem(idx, 'producto', e.target.value)}
                            className="w-full bg-background border border-slate-700 rounded-lg px-2 py-1.5 text-xs text-textLight focus:outline-none focus:border-primary mb-1">
                            <option value="">Seleccionar...</option>
                          ...
                          <div className="flex gap-1">
                            <input type="number" placeholder="Cant." value={item.cantidad} onChange={e => updateOrderItem(idx, 'cantidad', e.target.value)}
                              className="w-20 bg-background border border-slate-700 rounded px-2 py-1 text-xs text-textLight focus:outline-none focus:border-primary" min="1" />
                            <input type="number" placeholder="$ Unit." value={item.precioUnitario} onChange={e => updateOrderItem(idx, 'precioUnitario', e.target.value)}
                              className="flex-1 bg-background border border-slate-700 rounded px-2 py-1 text-xs text-textLight focus:outline-none focus:border-primary" min="0" />
          ...
          <div className="flex gap-3 p-5 border-t border-slate-800">
            <button onClick={() => setModal(null)} className="flex-1 px-4 py-2.5 rounded-lg border border-slate-700 text-textMuted hover:text-textLight transition-colors text-sm font-medium">Cancelar</button>
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="relative">
          ...
          <input type="text" placeholder="Buscar..." value={search} onChange={e => setSearch(e.target.value)}
            className="bg-background border border-slate-700 rounded-lg pl-9 pr-3 py-2 text-sm text-textLight focus:outline-none focus:border-primary w-64" />
        </div>
      </div>
          ...
          <table className="w-full text-left text-sm text-textLight">
            ...
            <tbody className="divide-y divide-slate-800">
              ...
              {filtered(suppliers).map(s => (
                <tr key={s._id} className={`hover:bg-slate-800/40 transition-colors ${!s.activo ? 'opacity-50' : ''}`}>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-center gap-2">
                      <button onClick={() => openModal(s)} className="p-1.5 text-blue-400 hover:bg-blue-500/10 rounded-lg transition-colors"><Edit3 size={15} /></button>
            ...
            <tbody className="divide-y divide-slate-800">
              ...
              {filtered(expenses).map(e => (
                <tr key={e._id} className="hover:bg-slate-800/40 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <span className="bg-slate-700/50 text-textMuted px-2 py-0.5 rounded text-xs capitalize">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-center gap-2">
                      <button onClick={() => openModal(e)} className="p-1.5 text-blue-400 hover:bg-blue-500/10 rounded-lg transition-colors"><Edit3 size={15} /></button>
          ...
          <table className="w-full text-left text-sm text-textLight">
            ...
            <tbody className="divide-y divide-slate-800">
              ...
              {filtered(customers).map(c => (
                <tr key={c._id} className={`hover:bg-slate-800/40 transition-colors ${!c.activo ? 'opacity-50' : ''}`}>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-center gap-2">
                      <button onClick={() => openModal(c)} className="p-1.5 text-blue-400 hover:bg-blue-500/10 rounded-lg transition-colors"><Edit3 size={15} /></button>
  ...
  <div>
    ...
    {textarea ? (
      <textarea value={value} onChange={e => onChange(e.target.value)} rows={3}
        className="w-full bg-background border border-slate-700 rounded-lg px-3 py-2 text-sm text-textLight focus:outline-none focus:border-primary resize-none" />
    ) : (
